...
export default class StaffDetail extends React.Component {
	...
	handlerEdit() {
		...
		item.key = this.props.staffDetail.id;
	...
	componentDidUpdate() {
		if (this.props.staffDetail == null) { }
			...
			for (let i = 0; i < selSex.options.length; i++) {
				if (selSex.options[i].value == this.props.staffDetail.rData.sex) {
			...
			for (let i = 0; i < selId.options.length; i++) {
				if (selId.options[i].value == this.props.staffDetail.rData.id) {
			...
			<div className="overLay">
				...
				<table ref="editTabel">
					<tbody>
						<tr>
							<th>姓名</th>
							<td><input id='staffEditName' type="text" defaultValue={staffDetail.rData.name}></input></td>
						</tr>
						<tr>
							<th>年龄</th>
							<td><input id='staffEditAge' type="text" defaultValue={staffDetail.rData.age}></input></td>
						</tr>
						<tr>
							<th>性别</th>
							<td>
						...
						<tr>
							<th>个人描述</th>
							<td><textarea id='staffEditDescrip' type="text" defaultValue={staffDetail.rData.descrip}></textarea></td>
						</tr>
					</tbody>
				</table>
